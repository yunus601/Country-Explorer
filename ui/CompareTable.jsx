...
function CompareTable({ countryA, countryB }) {
  ...
  const higherPopulation = Math.max(countryApopulation, countryBpopulation);
  ...
  return (
    <div className="@container mt-6 lg:px-16 lg:w-3/4 mx-auto text-xl tracking-wider">
      <div className="rounded-2xl border border-neutral-border bg-neutral-card-bg shadow-sm overflow-hidden grid grid-cols-[1fr_1fr_1fr] gap-x-4 gap-y-2 divide-y divide-neutral-border @lg:gap-x-6">
        ...
        <div className="col-span-3 grid grid-cols-subgrid py-4 px-4 bg-neutral-card-bg text-text-secondary @lg:py-5 @lg:px-6">
          ...
          <div
            className={` font-medium ${ishigherPopulation ? "text-brand-accent" : "text-gray-400"} text-2xl @lg:text-3xl uppercase`}
          >
            <span>{formatPopulation(countryA?.population)}</span>
            <div className="mt-5 w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
              <div
                style={{ width: `${rangeValueA}%` }}
                className={`h-full rounded-full ${ishigherPopulation ? "bg-brand-accent" : "bg-gray-400"}`}
              />
            </div>
          </div>
          <div
        ...
        <div className="col-span-3 grid grid-cols-subgrid py-4 px-4 bg-neutral-card-bg text-text-secondary @lg:py-5 @lg:px-6">
          ...
          <div className=" font-medium p-2 bg-brand-accent text-blue-50 w-1/2 rounded-lg text-center uppercase shadow-md">
            {countryA?.region}
          </div>
          <div className=" font-medium p-2 bg-brand-accent text-blue-50 w-1/2 rounded-lg text-center uppercase shadow-md">
            {countryB?.region}
        ...
        <div className="col-span-3 grid grid-cols-subgrid py-4 px-4 bg-neutral-card-bg text-text-secondary @lg:py-5 @lg:px-6">
          ...
          <div className=" font-medium">{countryA?.subregion}</div>
          <div className=" font-medium">{countryB?.subregion}</div>
        </div>
        <div className="col-span-3 grid grid-cols-subgrid py-4 px-4 bg-neutral-card-bg text-text-secondary @lg:py-5 @lg:px-6">
          <div className=" font-semibold text-text-primary">Capital</div>
          <div className=" font-medium">{countryA?.capitals?.[0]?.name}</div>
          <div className="font-medium">{countryB?.capitals?.[0]?.name}</div>
        </div>
        <div className="col-span-3 grid grid-cols-subgrid py-4 px-4 bg-neutral-card-bg text-text-secondary @lg:py-5 @lg:px-6">
          <div className=" font-semibold text-text-primary">Languages</div>
          <div className=" font-medium">
            {countryA?.languages.map((language) => language.name).join(", ")}
          </div>
          <div className="font-medium">
            {countryB?.languages.map((language) => language.name).join(", ")}
          </div>
        </div>
        <div className="col-span-3 grid grid-cols-subgrid py-4 px-4 bg-neutral-card-bg text-text-secondary @lg:py-5 @lg:px-6">
          <div className=" font-semibold text-text-primary">Currencies</div>
          <div className=" font-medium">{countryA?.currencies?.[0].code}</div>
          <div className="font-medium">{countryB?.currencies?.[0].code}</div>
        </div>
        <div className="col-span-3 grid grid-cols-subgrid py-4 px-4 bg-neutral-card-bg text-text-secondary @lg:py-5 @lg:px-6">
          <div className=" font-semibold text-text-primary">Timezones</div>
          <div className=" font-medium">{countryA?.timezones?.[0]}</div>
          <div className=" font-medium">{countryB?.timezones?.[0]}</div>
        </div>
      </div>
    </div>
  );
